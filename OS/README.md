...
### 2-4. 멀티 스레딩 시 스레드마다 스택과 PC를 독립적으로 할당하는 이유는 무엇인가요?
...
## 3. 멀티 스레드와 멀티 프로세스가 무엇이고, 각각의 장단점은 무엇인가요?
...
### 4-1. Critical Section(임계영역)과 Critical Section Problem(임계영역 문제)가 무엇인가요?
...
#### 4-2-1. 동기화를 제공하는 방식에 대해 설명해 주세요.
...
### 6-3. CPU 스케줄러(단기 스케줄러)의 종류에 대해 설명해 주세요.
...
## 7. 동기와 비동기, Blocking과 Non-Blocking은 무엇인가요?
...
### 9-3. 페이지 부재와 페이지 교체에 대해 설명해 주세요.

페이지 부재는 CPU에서 요청한 페이지가 현재 메모리에 없는 경우를 말합니다. 이 때 부재 페이지를 디스크에서 읽어와야 하는데 막대한 오버헤드가 발생합니다. 페이지 부재 발생 시 메모리에 해당 페이지를 적재해야 하지만 메모리가 꽉 찬 경우 가장 쓸모 없는 페이지를 쫓아내고 그 자리에 부재된 페이지를 적재하는 것이 페이지 교체입니다.

### 9-4. 페이지 교체 알고리즘은 어떤 것들이 있습니까?
...
#### 9-4-1. Page Reference String은 무엇인가요?
...
- [시스템 콜 예시](https://gyoogle.dev/blog/computer-science/operating-system/System Call.html)
