...
페이지의 사용량을 추적하는 방식에서 다릅니다. NUR 알고리즘은 LRU 알고리즘이 이론상으론 가장 최적의 성능을 제공하지만 실제 구현하는 데의 비용이 많기에 제시된 LRU 근사 알고리즘입니다.<br>
NUR 알고리즘은 주기적으로 참조비트를 클리어합니다. 그리고 해당 주기 안에 참조된 페이지는 참조비트를 1로 설정합니다. 페이지 부재 시 가장 낮은 우선 순위의 페이지들 가운데서 랜덤으로 페이지를 교체합니다. OS 특성상 구현 불가한 LRU 알고리즘을 대체하는데 사용됩니다.

> **추가 내용** : LRU가 사용되지 않는 이유<br>
> 이미 올라가 있는 페이지를 사용할 경우, 이미 특정 프레임에 올라가있기 때문에 주소 변환만 해서 접근하면 되고, 당연히 page fault가 발생하지 않습니다. 즉, I/O 작업의 필요성이 없기 때문에 OS 개입도 없습니다. 결국 실제 시스템에서는 이미 올라간 페이지가 언제 몇 번 사용됐는지 확인할 수 없습니다.
...
- 메모리 배치 전략 : 최초 적합, 다음 적합, 최적 적합, 최악 적합
- 유닉스와 리눅스
- 프로세스 생성 과정
- 요구 페이징의 페이저
- MMU 보충
