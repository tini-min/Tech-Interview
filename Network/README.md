...
컴퓨터 간에 정보를 주고 받을 때의 통신 방법에 대한 규칙이나 표준입니다.

### 1-3. 전이중 통신과 반이중 통신이 무엇입니까?
...
#### 1-5-1. 가상회선 패킷 교환과 데이터그램 패킷 교환은 무엇인가요?
...
### 1-6. 3-way 핸드셰이크와 4-way 핸드셰이크를 설명해 주세요.
...
#### 1-6-1. 3-way Handshake에서 서버도 클라이언트의 ACK 패킷을 기다리는 이유는 무엇인가요?

TCP는 양방향 통신을 지원하기 때문에 클라이언트가 서버에 데이터를 요청한 경우 서버에서 클라이언트에 데이터를 보낼 때 연결된 상태가 필요하기 때문입니다. <span style="color:red">(보충 필요)</span>

### 1-7. SYN Flooding이 무엇이며 이를 방어하는 방법은 무엇입니까?
...
### 1-8. TCP 연결이 수립되고 실제로 데이터를 보내는 방식을 설명하고 이를 효율적으로 하기 위해서 제안된 방식이 무엇인가요?
...
## 2. TCP/IP(Transmission Control Protocol / Internet Protocol) 모델이 무엇입니까?

OST 모델을 4계층으로 단순화 시켜서 사용하는 모델입니다. 응용 계층, 전송 계층, 인터넷 계층, 네트워크 접속 계층으로 나뉩니다.

## 3. HTTP와 HTTPS는 무엇인가요?
...
### 3-1. HTTP의 GET과 POST에 대해 설명해 주세요.
...
#### 3-1-1. 조회에 POST보다 GET이 사용되는 이유는 무엇인가요?
...
### 3-2. HTTP Method에서 PUT과 PATCH의 차이점을 설명해 주세요.

put -> 모두 교체<br>
patch -> 일정부분<br>
<span style="color:red">(보충 필요)</span>

### 3-3. 단방향 암호화와 양방향 암호화는 무엇인가요?

단방향 암호화 -> 암호화 ㅇ 복호화 ㄴ<br>
양방향 암호화 -> 암호화 ㅇ 복호화 ㅇ<br>
<span style="color:red">(보충 필요)</span>

#### 3-3-1. 대칭키와 공개키는 무엇입니까?
...
### 3-4. 쿠키와 세션이 무엇이며, 필요한 이유가 무엇인가요?
...
### 4-1. 로드 밸런서가 서버를 선택하는 방식을 설명해 주세요.
...
### 4-2. L4 로드 밸런서와 L7 로드 밸런서에 대해 설명해 주세요.
...
## 5. REST(REpresentational State Transfer)란 무엇인가요?
...
<span style="color:red">(보충 필요)</span> 자원의 표현 Json, XML 공부하기
